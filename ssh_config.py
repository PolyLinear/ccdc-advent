...
class SSHConfiguration(ccdc_script.CCDCScript):
    ...
    def produce_json(self) -> dict:
        ...
        if self.distro == ccdc_script.Distro.CCDC_RHEL:
            ...
            self.print_err("RHEL System: setting port label for SELINUX policy")
            semanage_proc = subprocess.run(['semanage', 'port', '-a',  '-t', 'ssh_port_t', '-p', 'tcp', '4444'], capture_output=True)
            if semanage_proc.returncode != 0:
                self.print_err(f"Failed to set port label: {semanage_proc.stderr.decode().rstrip()}")
