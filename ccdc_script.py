...
class CCDCScript(ABC):
    ...
    def enable_service(self, service: str) -> bool :
        if self.distro == Distro.CCDC_RHEL or self.distro == Distro.CCDC_DEBIAN:
            enabling = subprocess.run(["systemctl", "enable", "--no-pager", "--now", service], capture_output=True)
            return enabling.returncode == 0
        elif self.distro == Distro.CCDC_ALPINE:
            enabling = subprocess.run(["rc-update", "add", service], capture_output=True)
            return enabling.returncode == 0
        return False

    def restart_service(self, service: str) -> bool:
        if self.distro == Distro.CCDC_RHEL or self.distro == Distro.CCDC_DEBIAN:
            restart = subprocess.run(["systemctl", "restart", "--no-pager",  service], capture_output=True)
            return restart.returncode == 0
        elif self.distro == Distro.CCDC_ALPINE:
            restart = subprocess.run(["rc-service", "restart", service], capture_output=True)
            return restart.returncode == 0
        return False
